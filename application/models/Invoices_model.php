                            ...
                            foreach ($item['taxname'] as $taxname) {
                                if ($taxname != '') {
                                    ...
                                    if(isset($tax_array[0]) && isset($tax_array[1])){
                                        ...
                                        if(total_rows('tblitemstax',array('itemid'=>$itemid,'taxrate'=>$tax_rate,'taxname'=>$tax_name,'rel_id'=>$insert_id,'rel_type'=>'invoice')) == 0){
                    ...
                    foreach ($item['taxname'] as $taxname) {
                        if ($taxname != '') {
                            ...
                            if(isset($tax_array[0]) && isset($tax_array[1])){
                                $tax_name = trim($tax_array[0]);
                                $tax_rate = trim($tax_array[1]);
                                if(total_rows('tblitemstax',array('taxrate'=>$tax_rate,'taxname'=>$tax_name,'itemid'=>$new_item_added,'rel_id'=>$id,'rel_type'=>'invoice')) == 0){
                                    $this->db->insert('tblitemstax', array(
                                        'taxrate' => $tax_rate,
                                        'taxname' => $tax_name,
