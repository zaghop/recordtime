    ...
    public function get_gantt_data($project_id, $type = 'milestones')
    {
        ...
        foreach ($type_data as $data) {
            if ($type == 'milestones') {
                ...
                if ($data['staff_id'] != 0) {
                    $tasks = $this->get_tasks($project_id, 'tblstafftasks.id IN (SELECT taskid FROM tblstafftaskassignees WHERE staffid=' . $data['staff_id'] . ')', true);
                    $name  = get_staff_full_name($data['staff_id']);
                } else {
                    $tasks = $this->get_tasks($project_id, 'tblstafftasks.id NOT IN (SELECT taskid FROM tblstafftaskassignees)', true);
                    $name  = $data['name'];
                }
            } else {
                $tasks = $this->get_tasks($project_id, array(
